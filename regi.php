			...
			if (preg_match("/^[a-zA-Z]*$/", $fname) && preg_match("/^[a-zA-Z]*$/", $lname)) {
				if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
					...
					if ($Check_email_rows==0) {
						$check_contact = mysqli_query($con, "select * from student where contact='$contact'");
						$Check_contact_rows = mysqli_num_rows($check_contact);
						if ($Check_contact_rows==0) {
							$insert_reg = mysqli_query($con, "insert into student(fname, lname, idno, gander, contact, email, password) values('$fname','$lname','$idnumber','$gender','$contact','$email','$password')");
							if ($insert_reg) {echo "<script>alert(' Account created successful'); window.open('student-login.php','_self')</script>";
