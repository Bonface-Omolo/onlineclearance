...
<?php 
	...
	if (isset($_POST['edit'])) {
		...
		if ($noRow>0){
			$updateQuery = mysqli_query($con, "update clearance set finance='$approve' where id='$pID'");
			if ($updateQuery) {
				echo "<script>alert('Clearance submitted successfully'); window.open('view-finance.php','_self') </script>";
			}else{
