		...
		if ($password===$cpassword) {
			if (preg_match("/^[a-zA-Z]*$/", $fname) && preg_match("/^[a-zA-Z]*$/", $lname)) {
				if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
					$check_email = mysqli_query($con, "select * from lecturer where email='$email'");
					$Check_email_rows = mysqli_num_rows($check_email);
					if ($Check_email_rows==0) {
						$check_contact = mysqli_query($con, "select * from lecturer where contact='$contact'");
						$Check_contact_rows = mysqli_num_rows($check_contact);
						if ($Check_contact_rows==0) {
							$insert_reg = mysqli_query($con, "insert into lecturer(fname, lname, idno, gander, contact, email, password) values('$fname','$lname','$idnumber','$gender','$contact','$email','$password')");
