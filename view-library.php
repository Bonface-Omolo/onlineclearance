<?php session_start(); include('library-session.php'); ?>
...
<body>
	...
	<nav>
		<ul>
			...
			<li><a href="librarylogin.php">LOGOUT</a></li>
		</ul>
	</nav>
</header>
<div class="bgsize" >
			...
			?>
				...
				<tr>
					<form action="" method="post">
						...
						<td>
							<a href='library-edit.php?unitid=<?php echo $row['id']; ?>' type="submit" name="edit" value="Edit" style="text-decoration:none;padding:3px;border-radius:4px;border:none;background-color:steelblue;color:white">Clearance</a>
...
<?php 
	...
	if (isset($_POST['delete'])) {
		...
		$checkQuery = mysqli_query($con, "select * from clearance where id='$check' ");
		$noRow = mysqli_num_rows($checkQuery);

		if ($noRow>0) {
			$delete = mysqli_query($con, "delete from clearance where id='$check'");
			if ($delete) {
				echo "<script>alert('The row Deleted successfully'); window.open('view-library.php','_self') </script>";
			}else{
				echo "<script>alert('The row failed to delete'); window.open('view-library.php','_self') </script>";
			}
		}
	}
